    ...
    for _ in range(num_iterations):
        ...
        if len(lines) >= 11:
           ...
           # public_key_address = "D:\\项目\\DP+\\实施部署API\\Pre环境\\links_apiTest_publickey.asc"
            public_key_address = "D:\桌面\links_LINKSFIELD_PM_publickey.asc"
            Private_key_address = "D:\\桌面\\tianciprivate.asc"
            passwords = '123456'
            output_address = "D:\\项目\\DP+\\实施部署API\\测试环境\\profile_apiTest_45407_002.txt"
            ...
            print(data)
        '''
            data写入到D:\\项目\\DP+\\实施部署API\\测试环境\\profile.xlsx文件的B2单元格内，第二次循环写入到B3,第三次循环写入到B4，依次类推
